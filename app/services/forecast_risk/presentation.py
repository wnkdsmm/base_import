...
def _build_risk_notes(
    ...
    notes.append(
        "Итоговый риск теперь всегда раскладывается на четыре компонента: частоту пожаров, тяжесть последствий, долгое прибытие и дефицит водоснабжения."
    )
    notes.append(
        f"Текущий профиль весов: {weight_profile.get('mode_label') or 'Экспертные веса'}; для сельских территорий вес логистики и водоснабжения увеличивается автоматически."
    )
    notes.append(
    ...
    notes.append(
        f"Историческая проверка ranking: {historical_validation.get('status_label') or 'пока без проверки'}. Это черновая валидация, а не полноценный backtesting."
    )
    notes.append("Экспертный risk score не равен ни сценарному прогнозу по дням, ни ML-предсказанию; он показывает управленческий приоритет территории.")
