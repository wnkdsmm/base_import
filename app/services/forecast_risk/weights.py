...
from .utils import _clamp, _format_integer, _format_probability, _unique_non_empty
...
def _build_expert_retained_profile(
    ...
    notes = _unique_non_empty(list(profile.get("notes") or []) + [summary])
    calibration = deepcopy(profile.get("calibration") or {})
    expert_comparison = _build_metric_comparison(expert_entry.get("aggregate") or {}, expert_entry.get("aggregate") or {})
    calibration.update(
        {
            "ready": True,
            "used_fallback": True,
