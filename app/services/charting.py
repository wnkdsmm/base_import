...
def build_access_points_points_scatter_chart(rows: Sequence[Dict[str, Any]]) -> Dict[str, Any]:
    from app.services.access_points.charts_impl import _build_points_scatter_chart
...
def build_clustering_scatter_chart(
    ...
    from app.services.clustering.charts_impl import _build_scatter_chart
...
def build_clustering_distribution_chart(
    ...
    from app.services.clustering.charts_impl import _build_distribution_chart
...
def build_clustering_diagnostics_chart(
    ...
    from app.services.clustering.charts_impl import _build_diagnostics_chart
...
def build_forecasting_forecast_chart(
    ...
    from app.services.forecasting.charts_impl import _build_forecast_chart

    return _build_forecast_chart(list(daily_history), list(forecast_rows))
...
def build_forecasting_forecast_breakdown_chart(
    ...
    from app.services.forecasting.charts_impl import _build_forecast_breakdown_chart

    return _build_forecast_breakdown_chart(list(forecast_rows), recent_average)


def build_forecasting_weekday_chart(weekday_profile: Sequence[Dict[str, Any]]) -> Dict[str, Any]:
    from app.services.forecasting.charts_impl import _build_weekday_chart

    return _build_weekday_chart(list(weekday_profile))


def build_forecasting_geo_chart(geo_prediction: Dict[str, Any]) -> Dict[str, Any]:
    from app.services.forecasting.charts_impl import _build_geo_chart
...
def build_dashboard_sql_widget_bar_plotly(
    title: str,
    items: Sequence[Dict[str, Any]],
    empty_message: str,
    color_key: str,
