...
from .analysis import _build_access_point_rows, _build_typology_rows, _select_incomplete_points, _select_top_points
...
def get_access_points_page_context(
    *,
    table_name: str = "all",
    district: str = "all",
    year: str = "all",
    ...
    try:
        initial_data = get_access_points_data(
        ...
        summary = _build_summary(
            ...
            selected_district_label=request_state["selected_district"] if request_state["selected_district"] != "all" else "Все районы",
        ...
        initial_data = _empty_access_points_data(
            ...
            summary=summary,
            notes=[
                "Страница проблемных точек открыта в безопасном режиме из-за внутренней ошибки расчёта.",
                f"Техническая причина: {exc}",
            ],
            bootstrap_mode="resolved",
        )
        initial_data["charts"] = _empty_access_points_charts("График недоступен, пока не восстановлен расчёт ranking.")
    return {
...
def get_access_points_data(
    *,
    ...
    records, input_notes = _collect_access_point_inputs(
        request_state["source_tables"],
        district=selected_district,
    ...
    notes = _build_notes(metadata_notes, input_notes, rows, incomplete_points)
    if not rows:
        return _ACCESS_POINTS_CACHE.set(
            request_state["cache_key"],
            {
    ...
    return _ACCESS_POINTS_CACHE.set(request_state["cache_key"], payload)
